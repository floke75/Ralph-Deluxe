...
> **v1 plan — archived.** This plan was the original design specification used to build the Ralph Deluxe orchestrator. The system has since been revised by v2 (see `ralph-deluxe-v2-revision-plan 2.md`). Key changes in v2: (1) the handoff output now includes a freeform narrative briefing (`summary` + `freeform` fields) that serves as the primary context for the next iteration, replacing the structured-fields-only approach; (2) two selectable operating modes — `handoff-only` (default, narrative IS the memory) and `handoff-plus-index` (narrative + periodic knowledge indexer); (3) a telemetry module emitting JSONL events; (4) a single-file HTML dashboard with control plane (pause/resume, inject notes, skip tasks, settings); (5) a purpose-built HTTP server (`serve.py`) for dashboard writes.
>
> The original L1/L2/L3 compaction system described below remains in the codebase for backward compatibility but is superseded by the handoff-first approach in both v2 modes. The phases below (1-12) were completed as the v1 build; the v2 revision was implemented as PRs 1-7 on top of the existing codebase.
