#!/usr/bin/env bash
# plan-ops.sh — Plan loading, dependency-aware task selection, and plan mutation
#
# MODULE RESPONSIBILITIES:
#   - Load plan state from plan.json and expose helpers for selecting/updating tasks.
#   - Enforce dependency satisfaction when choosing the next runnable task.
#   - Apply bounded amendment batches (add/modify/remove) generated by coding handoffs.
#   - Report completion/remaining counts for orchestrator control flow and telemetry.
#
# DEPENDENCY SATISFACTION LOGIC:
#   - get_next_task() only returns tasks with status "pending".
#   - A pending task is runnable when every ID in depends_on resolves to an existing
#     task in "done" state.
#   - Tasks are evaluated in array order; the first runnable candidate is selected.
#   - Missing or non-done dependencies keep a task blocked until later iterations.
#
# DEPENDENCIES:
...
#
# DATA FLOW:
#   get_next_task() → scans tasks in order, returns first pending task whose
#                     depends_on set is fully satisfied by done tasks
...
#   - plan.json: rewritten in-place via temp-file + rename for status changes/amendments.
#   - plan.json.bak: overwritten backup created once per accepted amendment batch.
#   - .ralph/logs/amendments.log: append-only audit log for amendment accept/reject events.
...
# Return the first runnable task from plan.json.
#
# NEXT-TASK SELECTION BEHAVIOR:
#   - Candidate set is limited to tasks currently in "pending" status.
#   - A candidate is runnable only when all depends_on task IDs are in "done" status.
#   - Selection is deterministic: first runnable task in .tasks array order wins.
#   - Returns compact JSON for that task, or empty output if nothing is runnable.
#
# ORDERING / IDEMPOTENCY ASSUMPTIONS:
#   - Repeated calls without plan mutation return the same task (stable ordering).
#   - Callers are expected to mark returned tasks in_progress to avoid duplicate pickup.
#
...
#
# AMENDMENTS APPLICATION LOGIC:
#   - Entire amendment batch is rejected if it exceeds 3 entries.
#   - Within accepted batches, amendments are applied sequentially in array order.
#   - Invalid individual amendments are skipped; processing continues for remaining items.
#   - add may insert after a specific task ID (or append), modify merges fields, remove
#     drops matching tasks unless guardrails block the operation.
#
# ORDERING / IDEMPOTENCY ASSUMPTIONS:
#   - Ordering matters: each amendment observes prior mutations from the same batch.
#   - Function is not globally idempotent; replaying a batch can duplicate adds or
#     re-log events unless callers de-duplicate upstream.
#   - Safe reapplication is only guaranteed for no-op batches (e.g., empty amendments).
#
# SAFETY GUARDRAILS:
...
# SIDE EFFECTS:
#   - Creates/overwrites plan.json.bak before applying the first amendment in a batch.
#   - Rewrites plan.json for each accepted mutation using temp-file + rename.
#   - Appends ACCEPTED/REJECTED outcomes to .ralph/logs/amendments.log.
